...
class VisualizadorController {
    ...
    public function crearRegistro($idElemento, $valorElemento, $idElementoPadre, $idAtributo) {
        ...
        $nombreModuloLog = "visualizadores";
        $idModuloLog = $modulo->consultarIdModuloPorNombre($nombreModuloLog);

        $existeIdElemento = $this->existeIdElemento($idElemento);
        if ($existeIdElemento == FALSE) {
            $resp = $visualizador->crearRegistro($idElemento, $valorElemento, $idElementoPadre, $idAtributo);
            if ($resp == "Creado") {
                $notificacion->crearNotificacion('Registro ' . $idElemento . ' - ' . $idElementoPadre . ' creado.');
                $log->crearLog($idModuloLog, 'Registro ' . $idElemento . ' - ' . $idElementoPadre . ' creado.');
                return "Creado";
            } else {
                $notificacion->crearNotificacion('Error al crear el registro  ' . $idElemento . ' - ' . $idElementoPadre . '.');
                $log->crearLog($idModuloLog, 'Error al crear el registro ' . $idElemento . ' - ' . $idElementoPadre . '.');
                return "Error al crear";
            }
        } else {
            $notificacion->crearNotificacion('Error al crear el registro ' . $idElemento . ' - ' . $idElementoPadre . '. ID existe.');
            $log->crearLog($idModuloLog, 'Error al crear el registro ' . $idElemento . ' - ' . $idElementoPadre . '. ID existe.');
            return "Id elemento existe";
        }
    }

    public function existeIdElemento($idElemento) {
        $visualizador = new Visualizador();
        $existe = $visualizador->idElementoExiste($idElemento);
        if ($existe["idElemento"] !== NULL && $existe["idElemento"] !== "") {
            return TRUE;
        } else {
            return FALSE;
        }
    ...
    public function consultarIdAtributoPorNombreYConjunto($nombreAtributo, $idConjuntoIndicadores) {
        $visualizador = new Visualizador();
        $resp = $visualizador->consultarIdAtributoPorNombreYConjunto($nombreAtributo, $idConjuntoIndicadores);
        return $resp['idAtributo'];
    }
